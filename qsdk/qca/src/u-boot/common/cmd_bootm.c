...
#define BIG_KERNEL_VERSION 4000000  /* Init var for rollback prevention. */
...
#endif
#define CONFIG_ROVER_SECURITY 1
		...
		ulong *os_len)
{
	...
	int		os_noffset;
#endif
  const char *fit_uname_version = "config@1";
...
#if defined(CONFIG_ROVER_SECURITY)
#define CMD_SIZE 64

    printf ("Executing Rollback prevention test.\n");
    int status = 0;
    ...
    do {
      ...
      snprintf(runcmd, sizeof(runcmd), "verifyhw");
      if (run_command(runcmd, 0) == CMD_RET_SUCCESS) {
          // Fuse is blown, so perform rollback prevention.
          doCheck = 1;
          printf ("Fuse has been blown.\n");
      } else {
          printf ("Fuse still in tact.\n");
          // Skip rollback prevention test
          status = 0;
          break;
      }

      cfg_noffset = fit_conf_get_node (fit_hdr, fit_uname_version);
      if (doCheck == 1 && cfg_noffset < 0) {
        // Wasn't able to find the version in kernel image.
        printf ("Error: Wasn't able to read kernel version. Booting stopped.\n");
        status = -1;
        break;
      }

      // Extract the description field.
      ret = fit_get_desc (fit_hdr, cfg_noffset, &desc);

      if (ret == 0) {
        uint kernel_version = 0;
        uint virtual_counter = BIG_KERNEL_VERSION;
        char *endptr = NULL;
        int ret_val = 0;

        // Found version in text format.
        // Convert char* to int.
        if (desc[0] =='\0') {
            printf("Error: kernel version is empty string.\n");
            status = -1;
            break;
        }

        kernel_version = (unsigned int) simple_strtoul (desc, &endptr, 10);
        if (*endptr != '\0') {
            printf("Error: Can't read kernel version string.\n");
            status = -1;
            break;
        }
        ...
        // Perform rollback detection.
        if (doCheck == 1) {
          if (ret_val < 0) {
            printf ("Error: Wasn't able to read virtual counter. Booting stopped.\n");
            status = -1;
            break;
          }

          if (virtual_counter > kernel_version) {
            printf ("Error: Kernel rollback has been detected. Booting stopped.\n");
            status = -1;
            break;
          }
        }
    ...
    if (status == -1) {
      ...
      // Notify users with a LED patttern. To be finalized pending UX design
      // Turn off white color
      run_command("i2c led 5 0", 0);
      // Add some delay
      udelay(100000);
      // Turn on amber color
