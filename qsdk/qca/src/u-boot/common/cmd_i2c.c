...
#if defined(CONFIG_ROVER_I2C)
#include <rover_i2c_leds.h>
#endif
...
#define LED_MAX_TIME_PER_STEP	31
#define LED_MAX_INITIAL_CURRENT	30
...
static int do_i2c_led ( cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	led_id = simple_strtoul(argv[1], NULL, 16);
	if (led_id > LED_ID_MAX)
		return CMD_RET_USAGE;
	brightness = simple_strtoul(argv[2], NULL, 16);
	if (brightness > LED_MAX_BRIGHTNESS)
		return CMD_RET_USAGE;
	printf("led_id=%d brightness=%d\n", led_id, brightness);
	if ((res = rover_i2c_led_set(led_id, brightness)) != 0) {
		printf("Command fails [%d]\n", res);
		return CMD_RET_FAILURE;
	}
	return 0;
}
static int do_i2c_led_up ( cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	int led_id;
	int brightness;
	int time_per_step = 1;
	...
	if (brightness > LED_MAX_BRIGHTNESS)
		return CMD_RET_USAGE;
	initial_current = simple_strtoul(argv[3], NULL, 16);
	if (initial_current > LED_MAX_INITIAL_CURRENT)
		return CMD_RET_USAGE;
	time_per_step = simple_strtoul(argv[4], NULL, 16);
	if (time_per_step > LED_MAX_TIME_PER_STEP)
		return CMD_RET_USAGE;

	printf("led_id=%d brightness=%d initial_current=%d time_per_step=%d\n",
			led_id, brightness, initial_current, time_per_step);
	if ((res = rover_i2c_led_dim_up(led_id, brightness,
			initial_current, time_per_step)) != 0) {
		printf("Command fails [%d]\n", res);
		return CMD_RET_FAILURE;
	}
	return 0;
...
static int do_i2c_led_down ( cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	int led_id;
	int brightness;
	int dest_current = 0;
	...
	if (brightness > LED_MAX_BRIGHTNESS)
		return CMD_RET_USAGE;
	dest_current = simple_strtoul(argv[3], NULL, 16);
	if (dest_current > LED_MAX_INITIAL_CURRENT)
		return CMD_RET_USAGE;
	time_per_step = simple_strtoul(argv[4], NULL, 16);
	if (time_per_step > LED_MAX_TIME_PER_STEP)
		return CMD_RET_USAGE;

	printf("led_id=%d brightness=%d dest_current=%d time_per_step=%d\n",
			led_id, brightness, dest_current, time_per_step);

	if ((res = rover_i2c_led_dim_down(led_id, brightness,
			dest_current, time_per_step)) != 0) {
		printf("Command fails [%d]\n", res);
		return CMD_RET_FAILURE;
	}
	return 0;
...
#if defined(CONFIG_ROVER_I2C)
	U_BOOT_CMD_MKENT(led, 3, 1, do_i2c_led, "", ""),
	U_BOOT_CMD_MKENT(led_up, 5, 1, do_i2c_led_up, "", ""),
	U_BOOT_CMD_MKENT(led_down, 5, 1, do_i2c_led_down, "", ""),
#endif
};
