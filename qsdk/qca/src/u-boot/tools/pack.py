...
class EmmcScript(FlashScript):
    ...
    def erase(self, offset, size):
        """Generate code, to erase the specified partition. In case size > 0x100000, break it to smaller erase size"""
        max_size = 1048576
        if size <= max_size:
            self.append("mmc erase 0x%08x %x" % (offset, size))
        else:
            blk_cnt = size / max_size
            blk_remain = size % max_size 
            for i in range(0, blk_cnt):
                self.append("mmc erase 0x%08x %x" % (offset + i * max_size, max_size))
            if blk_remain > 0:
                self.append("mmc erase 0x%08x %x" % (offset + blk_cnt * max_size, blk_remain))
...
class ArgParser(object):
    ...
    def parse(self, argv):
        ...
        try:
            opts, args = getopt(argv[1:], "Bib:hp:t:o:c:m:f:F:M:S:")
        ...
        self.signimg_fname = signimg_fname
    ...
    def usage(self, msg):
        ...
        print "   -S         specifies script name to sign images"
        print
...
def main():
    ...
    if parser.signimg_fname:
       prc = subprocess.Popen(['sh', parser.signimg_fname])
       prc.wait()

       if prc.returncode != 0:
          error('Unable to sign images')
       else:
          print 'Finished signing images'
