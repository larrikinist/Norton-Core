...
int is_component_authenticated(char *src, char *sig, char *cert)
{
	...
	remove(computed_file);
	remove(reference_file);
	if (memcmp(computed_hash, reference_hash, HASH_SIZE)) {
		free(computed_hash);
		free(reference_hash);
		return 1;
	}
...
#define expected_ca_hash_file "/etc/ca_hash"
#define ca_cert_file "/tmp/ca_cert"
#define ca_hash_file "/tmp/ca_hash"
#define CMD_SIZE 256
int verify_ca_cert_hash(struct image_section *section)
{
	Mbn_Hdr *mbn_hdr;
	int fd;
	uint8_t *fp;
	...
	// signing cert size
	cert_size = ( (fp[cert_offset + 2] << 8) | fp[cert_offset + 3] ) + 4;
	...
	fd = open(ca_cert_file, O_CREAT | O_WRONLY, S_IRUSR | S_IWUSR);
	...
	write(fd, fp + cert_offset, cert_size);

	close(fd);

	// compute ca cert hash
	snprintf(cmd, sizeof(cmd), "cat %s | openssl dgst -sha256 -binary -out %s", ca_cert_file, ca_hash_file);
	...
	expected_hash = read_file(expected_ca_hash_file);
	computed_hash = read_file(ca_hash_file);

	// compared computed hash with expecte hash
	if (memcmp(computed_hash, expected_hash, HASH_SIZE)) {
		printf("Failed to verify ca cert hash.\n");
		free(expected_hash);
		free(computed_hash);
		return -1;
	}

	printf("ca cert hash is verified.\n");
...
int is_image_authenticated(void)
{
	...
	for (i = 0; i < NO_OF_SECTIONS; i++) {
		...
		if (verify_ca_cert_hash(&sections[i]) != 0) {
			printf("%s has invalid ca cert.\n", sections[i].file);
			return 0;
		}
