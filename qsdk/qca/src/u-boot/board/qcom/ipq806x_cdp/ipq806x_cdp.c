...
static void gpio_pull_high(int n, char *desc)
{
	printf("Pull High for GPIO %d (%s)\n", n, desc);
	gpio_tlmm_config(n, 0, 1, GPIO_PULL_UP, GPIO_10MA, GPIO_OE_ENABLE);
}

static void init_gpio(void)
{
	printf("Initialize some PIOs to high\n");

	gpio_pull_high(6, "UART TX data");
	gpio_pull_high(15, "Z-Wave reset");
	gpio_pull_high(22, "USB 1 Enable");
	gpio_pull_high(23, "USB 2 Enable");
	gpio_pull_high(24, "USB 1 Fault N");
	gpio_pull_high(25, "USB 2 Fault N");
	gpio_pull_high(54, "Reserved");
	gpio_pull_high(55, "UART TX Data");
	gpio_pull_high(56, "UART RX Data");
	gpio_pull_high(65, "BT reset");
}
...
int board_init()
{
	...
	if (sfi->flash_type == SMEM_BOOT_NAND_FLASH) {
		...
		else if (gd->bd->bi_arch_number == MACH_TYPE_IPQ806X_AP161 ) {
			gboard_param->emmc_gpio = emmc1_gpio;
			gboard_param->emmc_gpio_count = ARRAY_SIZE(emmc1_gpio);
			printf("Enabling eMMC on AP161 reference design\n");
		}
...
void reset_cpu(ulong addr)
{
	...
	printf("\nResetting with watch dog!\n");
 	...
 	printf("wdt: APCS_WDT0_EN:%#08x APCS_WDT0_BITE_TIME:%#08x APCS_WDT0_STS:%#08x\n",
 		*((unsigned int *)APCS_WDT0_EN), *((unsigned int *)APCS_WDT0_BITE_TIME),
 		*((unsigned int *)(APCS_WDT0_EN+4)) );
	...
	printf("wait: APCS_WDT0_BITE_TIME:%#08x APCS_WDT0_STS:%#08x\n",
		*((unsigned int *)APCS_WDT0_BITE_TIME), *((unsigned int *)(APCS_WDT0_EN+4)) );
...
#if 1
		printf("Don't get MAC from ART partition of eMMC. It will hang!\n");
		return -1;
#endif
		/*
		 * ART partition 0th position (6 * 4) 24 bytes will contain the
...
int board_eth_init(bd_t *bis)
{
	...
	gpio_func_data_t *s17_gpio = gboard_param->reset_switch_gpio;
...
static void ipq_pci_gpio_fixup(void)
{
	...
	if (machid == MACH_TYPE_IPQ806X_RUMI3) {
		...
		gpio_2->gpio = PCIE_1_RST_GPIO;
				...
				(machid == MACH_TYPE_IPQ806X_AP161 )) {	/* For Rover */
		gpio_2->gpio = -1;
	}
}
