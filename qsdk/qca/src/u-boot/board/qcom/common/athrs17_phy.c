...
int ipq_athrs17_init(ipq_gmac_board_cfg_t *gmac_cfg)
{
	...
	if (gboard_param->reset_switch_gpio) {
		printf("Release switch reset\n");
		int  count;
		gpio_func_data_t *s17_gpio = gboard_param->reset_switch_gpio;
		if(s17_gpio) {
			/* release reset pin. Venus comapatibility */
			gpio_tlmm_config(s17_gpio->gpio, 0, 0, 1, 0, 0);
		} else {
			printf("there is no S17c reset pin defined\n");
		}
		/* wait 10 ms until switch up
		 * packet time on wire is 0.840 usec (64 bytes + 20 overhead + 8/10b encoding)
		 * it is possible to leak some packets from wan to lan here
		 */
		mdelay(25);
	}
	...
	if (ret != -1) {
		...
		/* configure vlan's before lan mac's/phy's to avoid lan-wan leak */
