...
static int qup_i2c_wait_writeready(struct qup_i2c_dev *qup)
{
	...
	u32 opflags;
	u32 status;
	u32 byte_sleep_factor;

	timeout = jiffies + HZ;
	if (qup->msg->addr == 0) {
		byte_sleep_factor = ((qup->slow_clk_ctl & 0xff) + 3)/((qup->clk_ctl & 0xff) + 3);
	}
	for (;;) {
		opflags = readl(qup->base + QUP_OPERATIONAL);
		...
		if (qup->msg->addr != 0)
			...
			usleep_range(qup->one_byte_t * byte_sleep_factor, qup->one_byte_t * 2 * byte_sleep_factor);
		}
	}
}
...
static int qup_i2c_write_one(struct qup_i2c_dev *qup, struct i2c_msg *msg)
{
	...
	if (msg->addr == 0)
		writel(qup->slow_clk_ctl, qup->base + QUP_I2C_CLK_CTL);
	else
...
static int qup_i2c_probe(struct platform_device *pdev)
{
	...
	fs_div = ((src_clk_freq / slow_clk_freq) / 2) - 3;
	qup->slow_clk_ctl = (hs_div << 8) | (fs_div & 0xff);
	/*
