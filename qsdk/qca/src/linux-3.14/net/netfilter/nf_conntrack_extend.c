		 ...
		 size_t var_alloc_len, gfp_t gfp)
{
	...
	p_ext = kzalloc(alloc_size, gfp);
	if (!p_ext)
		return NULL;
		
	rcu_assign_pointer(*ext, p_ext);
			     ...
			     size_t var_alloc_len, gfp_t gfp)
{
	...
	/* borisp - __realloc() allocates and copies memory w/o lock
		we always use new pointer */
	new = kzalloc(newlen, gfp);
	if (!new) {
		rcu_read_unlock();
		return NULL;
	}
	...
	new->len = newlen;
	rcu_read_unlock();
	/* Free pointer by the end of function */ 
	kfree_rcu(old, rcu);
