			   ...
			   unsigned int type, int value)
{
	...
	printk("!!!seen=%ld (%ld) event type=%u, code=%u, value=%d\n",
		seen, (seen - priv->seen) / HZ, type, data->b->code, value);
	
	/* Filter out possible sporadic interrupts */
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if (ret)
			...
			dev_info(&pdev->dev, "!!!gpio:%d has irq:%d Value=%d\n", button->gpio, button->irq,
